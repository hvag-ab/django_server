...
要将MongoDB与asyncio或Tornado一起使用，请参阅Motor项目。
...
这将污染文档的属性名称空间，因此当使用与字典方法相同的名称的键时可能导致细微的bugs/混淆错误。
我们使用SON对象而不是常规字典的唯一原因是维护键排序，因为服务器需要这些准确的操作。因此我们在是否需要复杂的SON的必要性上犹豫不决(某种程度上我们希望恢复单独使用字典，而不会破坏每个人的向后兼容性)
因为文档的表现像字典，所以新用户很容易(和Pythonic)的处理文档。如果我们开始改变这点将会为新受增加一个障碍-另外的学习成本。
